...
String HelpText()
{
    return String()
        ...
        << "4. The Client ID shown on that app's page.\n"
        ...
        << "Open in Spotify — opens the selected track or playlist in Spotify. It is navigation, not playback control. True Spotify player control is a separate API capability and permission.\n\n"
...
public:
    ...
    PlaylistHelpDialog()
    {
        ...
        body_.SetTextUtf8(HelpText());
        body_.SetReadOnly();
        body_.SetAcceptsTabs(false);
        body_.SetAcceptsDrop(false);
    ...
    virtual void Paint(Draw& w) override
    {
        ...
        w.DrawRect(GetSize(), mode == UiThemeMode::Dark ? Color(18, 18, 18) : SColorFace());
    }
    ...
    virtual void Layout() override
    {
        ...
        int button_h = DPI(34);
        title_.SetRect(margin, margin, w, DPI(70));
        int body_y = margin + DPI(70) + gap;
        int button_y = max(body_y, rc.GetHeight() - margin - button_h);
        body_.SetRect(margin, body_y, w, max(0, button_y - body_y - gap));
        button_row_.SetRect(margin, button_y, w, button_h);
    }
